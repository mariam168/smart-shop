            ...
            <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        {variations.map((variation) => { 
                            ...
                            return ( 
                                <div key={variation._id}> 
                                    <h3 className="mb-3 text-sm font-bold text-zinc-800 dark:text-white uppercase tracking-wider">{variationName}: <span className="text-zinc-500 dark:text-zinc-400 font-medium capitalize">{currentSelectedOption ? currentSelectedOption.name : ''}</span></h3> 
                                        ...
                                        {(variation.options || []).map((option) => { 
                                            ...
                                            return ( 
                                                <button key={option._id} onClick={() => handleOptionSelect(variation._id, option)} className={`group relative flex min-w-[48px] items-center justify-center gap-2 rounded-xl border-2 p-3 text-sm font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-light dark:focus:ring-offset-zinc-950 ${isSelected ? 'border-primary bg-primary/10 text-primary-dark dark:text-primary-light shadow-sm' : 'border-zinc-300 bg-white hover:border-primary-light dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-primary'}`}> 
                                                    {variation.name?.toLowerCase() === 'color' && colorMap[option.name?.toLowerCase()] && (<span style={{ backgroundColor: colorMap[option.name?.toLowerCase()] }} className="h-6 w-6 rounded-full border border-black/10 shadow-inner"></span>)} 
                ...
                <section id="reviews-section" className="py-16"> 
                    ...
                    <div className="mt-8"> 
                        ...
                        {activeTab === 'specifications' && attributes.length > 0 && ( <dl className="space-y-4 pt-2 max-w-2xl">{attributes.filter(attr => attr.key && attr.value).map((attr, index) => (<div key={index} className="grid grid-cols-1 gap-1 sm:grid-cols-3 sm:gap-4"><dt className="font-medium text-zinc-500 dark:text-zinc-400">{attr.key}</dt><dd className="col-span-2 font-semibold text-zinc-700 dark:text-zinc-300">{attr.value}</dd></div>))}</dl> )} 
