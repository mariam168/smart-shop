...
const ProductDetails = () => {
    ...
    const fetchProductDetails = useCallback(async () => {
        ...
        try {
            ...
            const descriptionExists = productData.description && (productData.description.en || productData.description.ar);
    ...
    return (
        <div className="bg-white dark:bg-zinc-950">
            <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                {isOfferProduct && ( <div className="relative overflow-hidden mb-8 p-8 rounded-2xl bg-gradient-to-br from-primary via-primary-dark to-zinc-900 text-white shadow-2xl shadow-primary/30"> <div className="flex flex-col md:flex-row justify-between items-center gap-6"> <div className="text-center md:text-left"> <h2 className="text-3xl font-bold flex items-center gap-3 [text-shadow:_0_2px_4px_rgb(0_0_0_/_30%)]"><Award size={32} />{advertisement.title || t('productDetailsPage.specialOffer')}</h2> <p className="mt-2 text-blue-100 max-w-lg [text-shadow:_0_1px_2px_rgb(0_0_0_/_20%)]">{advertisement.description}</p> </div> {timeLeft && (<div className="text-center shrink-0"><p className="text-sm uppercase font-semibold tracking-wider text-blue-200 flex items-center justify-center gap-2"><Clock size={16}/>{t('productDetailsPage.offerEndsIn')}</p><div className="flex gap-2 sm:gap-4 mt-2 font-mono text-3xl font-bold"><div>{String(timeLeft.days).padStart(2, '0')} <span className="text-sm block font-normal opacity-80">{t('general.days')}</span></div>:<div>{String(timeLeft.hours).padStart(2, '0')} <span className="text-sm block font-normal opacity-80">{t('general.hours')}</span></div>:<div>{String(timeLeft.minutes).padStart(2, '0')} <span className="text-sm block font-normal opacity-80">{t('general.minutes')}</span></div>:<div>{String(timeLeft.seconds).padStart(2, '0')} <span className="text-sm block font-normal opacity-80">{t('general.seconds')}</span></div></div></div>)} </div> </div> )}
                ...
                <main className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 pb-24"> <div className="lg:sticky lg:top-24 self-start flex flex-col-reverse md:flex-row gap-4"> <div className="flex flex-row md:flex-col gap-3 justify-center">{allImages.slice(0, 5).map(imgUrl => (<button key={imgUrl} className={`group relative flex-shrink-0 w-20 h-20 bg-zinc-100 dark:bg-zinc-900 rounded-xl cursor-pointer overflow-hidden transition-all duration-300 border-2 hover:border-primary-light focus:outline-none ring-offset-2 ring-offset-white dark:ring-offset-zinc-950 focus:ring-2 focus:ring-primary ${mainDisplayImage === imgUrl ? 'border-primary' : 'border-zinc-200 dark:border-zinc-800'}`} onClick={() => handleImageSelect(imgUrl)}><img src={imgUrl} alt="thumbnail" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"/></button>))}</div> <div className="relative w-full aspect-square bg-white dark:bg-zinc-900/50 rounded-2xl flex items-center justify-center overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-900/5"> 
                    ...
                    {description && <p className="mt-6 text-zinc-600 dark:text-zinc-400 leading-relaxed max-w-prose">{description.substring(0, 150).replace(/<[^>]*>?/gm, '')}{description.length > 150 ? '...' : ''}</p>} 
                        ...
                        {variations.map((variation) => { 
                            ...
                            return ( 
                                <div key={variation._id}> 
                                    <h3 className="mb-3 text-sm font-bold text-zinc-800 dark:text-white uppercase tracking-wider">{variationName}: <span className="text-zinc-500 dark:text-zinc-400 font-medium capitalize">{currentSelectedOption ? (language === 'ar' ? currentSelectedOption.name_ar : currentSelectedOption.name_en) : ''}</span></h3> 
